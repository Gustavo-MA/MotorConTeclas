...
const char* mqtt_server = "10.186.100.137"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
IPAddress server(10,186,100,137);

// Objetos
WiFiClient espClient; // Este objeto maneja los datos de conexion WiFi
PubSubClient client(espClient); // Este objeto maneja los datos de conexion al broker
Servo myservo1,myservo2,myservo3,myservo4; // Este objeto maneja las funciones del servomotor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  for (int i = 2; i < 11; i++) {  // Se imprime y concatena el mensaje
    Serial.print((char)message[i]);
    messageTemp += (char)message[i];
  }
  String contador=messageTemp;
    String messageTemp2; // Se declara la variable en la cual se generará el mensaje completo  
  for (int i = 12; i < length-1; i++) {  // Se imprime y concatena el mensaje
    Serial.print((char)message[i]);
  ...
  String val=limpiarSoloNumeros(messageTemp2);
  grados=val.toInt();
  ...
  if (String(topic) == "codigoiot/ejemplo/servoRobot") {  // En caso de recibirse mensaje en el tema codigoIoT/ejemplo/mqttin
    if(contador=="contador1")
    myservo1.write (grados);
    if(contador=="contador2")
    myservo2.write (grados);
    if(contador=="contador3")
    myservo3.write (grados);
    if(contador=="contador4")
    myservo4.write (grados);
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("codigoiot/ejemplo/servoRobot"); // Esta función realiza la suscripción al tema
...
String limpiarSoloNumeros(String s) {
  String resultado = "";
  for (int i = 0; i < s.length(); i++) {
    if (isDigit(s[i])) {   // Mantiene solo números
      resultado += s[i];
    }
  }
  return resultado;
